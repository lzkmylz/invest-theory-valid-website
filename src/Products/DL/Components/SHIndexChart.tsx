...
@observer
class SHIndexChart extends React.Component {
  componentDidMount() {
    this.updateChart();
  }
  ...
  updateChart = () => {
    ...
    Highcharts.chart('sh-index-chart', options);  
  }

  render() {
    if(DLStore.SHIndexData.data.length > 0) {
      this.updateChart();
    }
    return (
      <div id="sh-index-chart" ></div>
    );
  }
}

export default SHIndexChart;
